...
Can hh;
//创建按钮
Button newGame((windowsLenth1-240)/2, windowsWidth1 / 5, 240, 125);
Button continueGame((windowsLenth1 - 240) / 2, 2 * windowsWidth1 / 5, 240, 125);
Button achievement((windowsLenth1 - 240)/2, 3 * windowsWidth1 / 5, 240, 125);
Button back(windowsLenth1-100, 0, 100, 60);
Button shop((windowsLenth1 - 240) / 2,4 * windowsWidth1 / 5,240, 125);
...
Title shop_t((windowsLenth1 - 240) / 2, windowsWidth1 / 15, 160, 90);
...
{         
    ...
    HBITMAP shop240125 = (HBITMAP)LoadImage(NULL, "shop240125.bmp", IMAGE_BITMAP, 0, 0, LR_LOADFROMFILE | LR_CREATEDIBSECTION);
    ...
    {   
            ...
            if (inputRecord.Event.MouseEvent.dwButtonState == FROM_LEFT_1ST_BUTTON_PRESSED)//左键点击
            {
                ...
                case BEFORE_START://这个界面的按钮有：开始，继续，成就，退出，设置（？）
                {
                   ...
                   /* cout << " x:" << p.x << " y:" << p.y << endl;
                    ...
                    else if (isKickButton(p.x, p.y, shop, 0))
                    {
                        isFirst = true;
                        flag = SHOP_SHOW;
                    }
        ...
        case BEFORE_START:
        {
            ...
            //BEFOR_START是游戏初始界面  画到内存dc上
            DrawBmp(hDC, dcMEM, newGame240125, newGame, 240, 125, isFirst);
            DrawBmp(hDC, dcMEM, continue240125, continueGame, 240, 125, isFirst);
            DrawBmp(hDC, dcMEM, achievement240125, achievement, 240, 125, isFirst);
            DrawBmp(hDC, dcMEM, shop240125, shop, 240, 125, isFirst);
            break; 
        }
        case BEFORE_INIT:
        {
