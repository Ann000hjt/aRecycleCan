...
{         
   ...
   //定义缓冲区大小 
    ...
    HDC cmdmem = CreateCompatibleDC(hDC);//创建一个与屏幕显示DC兼容的内存设备上下文环境（DC）
    ...
    {   
        ...
        if (inputRecord.EventType == MOUSE_EVENT)     //如果当前为鼠标事件  
        {
            if (inputRecord.Event.MouseEvent.dwButtonState == FROM_LEFT_1ST_BUTTON_PRESSED)//左键点击
            {
                ...
                case BEFORE_START://这个界面的按钮有：开始，继续，成就，退出，设置（？）
                {
                    ...
                    //if(p.x > windowsLenth / 2 - 280 && p.x < windowsLenth / 2 +40 && p.y > windowsWidth / 3 - 90 && p.y < windowsWidth / 3 + 90)
           
                    if(isKickRect(p.x,p.y, windowsLenth / 2 - 120,windowsWidth/3,320,180,8))
                    {
                        system("cls");
        ...
        case BEFORE_START:
        {
            ////把开始键按钮移动到她该呆着的位置  这里借hh来放开始按钮
            //hh.setX(windowsLenth / 2-120);//调整了这个数值让按钮待在中间 但每个电脑不同
            //hh.setY(windowsWidth/3);
            //DrawBmp(hDC,dcMEM, newGame320180, hh, 320, 180);//画到内存dc上
            SelectObject(cmdmem, newGame320180);
            BitBlt(hDC, windowsLenth/2-120, windowsWidth/3, 320, 180, cmdmem, 0, 0, SRCCOPY);
            break;
        }
        case BEFORE_INIT:
        {
        ...
        case PLAYING_GAME:
        {
            ...
            //draw
            DrawBmp(hDC,dcMEM, can180320, hh, 180, 320,interpolation);
            break;
        }
